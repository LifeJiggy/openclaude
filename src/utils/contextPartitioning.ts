...
/**
 * Classify message into priority zone.
 */
...
function calculateImportance(message: Message): number {
  ...
  if (content.includes('error') || content.includes('fail')) score += 0.3
  if (content.includes('important') || content.includes('critical')) score += 0.2
  if (content.includes('tool_use')) score += 0.2
  if (message.message?.role === 'user') score += 0.1
  if (content.includes('decision') || content.includes('conclusion')) score += 0.2
...
/**
 * Partition messages into priority zones.
 */
export function partitionContext(
  ...
  for (const msg of olderMessages) {
    ...
    if (zone === 'important' && zoneTokens.get('important')! < (options.zones?.[1]?.maxTokens ?? 30000)) {
      currentZone.push(msg)
      zoneTokens.set('important', zoneTokens.get('important')! + roughTokenCountEstimation(
        typeof msg.message?.content === 'string' ? msg.message.content : ''
      ))
    } else if (zone === 'background' && zoneTokens.get('background')! < (options.zones?.[2]?.maxTokens ?? 10000)) {
      currentZone.push(msg)
...
/**
 * Get messages from specific zone.
 */
...
/**
 * Get combined messages across all zones.
 */
