...
describe('contextPartitioning', () => {
  describe('partitionContext', () => {
    it('partitions messages into zones', () => {
      const messages = [
        ...
        createMessage('user', 'Help me with code'),
    ...
    it('tracks zone tokens', () => {
      const messages = [
        createMessage('user', 'Short message'),
        createMessage('assistant', 'Another short message'),
      ]

      const context = partitionContext(messages, { contextWindow: 100000 })

      expect(context.zoneTokens.get('background')).toBeGreaterThan(0)
    })
  })
  ...
  describe('getAvailableSpace', () => {
    it('returns non-negative available space', () => {
      const messages = [
